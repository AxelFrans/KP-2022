...
class Transaksi extends CI_Controller
{
	...
	public function index()
	{
		...
		if ($this->session->userdata('level') == 'Anggota') {
			$this->data['pinjam'] = $this->db->query(
				"SELECT DISTINCT `pinjam_id`, `nisn`, 
				`status`, `tgl_pinjam`, `lama_pinjam`, `tgl_balik`, `tgl_kembali` 
				FROM tbl_pinjam WHERE status = 'Dipinjam' 
				AND nisn = ? ORDER BY pinjam_id DESC",
				array($this->session->userdata('nisn'))
			);
		} else {
			$this->data['pinjam'] = $this->db->query("SELECT DISTINCT `pinjam_id`, `nisn`, 
				`status`, `tgl_pinjam`, `lama_pinjam`, `tgl_balik`, `tgl_kembali` 
	...
	public function kembali()
	{
		...
		if ($this->session->userdata('level') == 'Anggota') {
			$this->data['pinjam'] = $this->db->query("SELECT DISTINCT `pinjam_id`, `nisn`, 
				`status`, `tgl_pinjam`, `lama_pinjam`, `tgl_balik`, `tgl_kembali` 
				FROM tbl_pinjam WHERE nisn = ? AND status = 'Di Kembalikan' 
				ORDER BY id_pinjam DESC", array($this->session->userdata('nisn')));
		} else {
			$this->data['pinjam'] = $this->db->query("SELECT DISTINCT `pinjam_id`, `nisn`, 
				`status`, `tgl_pinjam`, `lama_pinjam`, `tgl_balik`, `tgl_kembali` 
	...
	public function detailpinjam()
	{
		...
		if ($this->session->userdata('level') == 'Anggota') {
			...
			if ($count > 0) {
				...
				AND nisn =?",
					array($id, $this->session->userdata('nisn'))
				)->row();
			} else {
	...
	public function result()
	{
		...
		$user = $this->M_Admin->get_tableid_edit('tbl_login', 'nisn', $this->input->post('kode_anggota'));
		error_reporting(0);
		if ($user->nama != null) {
